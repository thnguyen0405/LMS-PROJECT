...
if(isset($_POST['form_submit'])) {
    try  {
        ...
        $statement->execute(array($_REQUEST['course_id'], $_REQUEST['student_id'], $_REQUEST['instructor_id'], 'instructor', $_POST['message'], date('Y-m-d H:i:s')));

        // Getting student email
        $statement = $pdo->prepare("SELECT * FROM students WHERE id=?");
        $statement->execute([$_REQUEST['student_id']]);
        $student_data = $statement->fetch(PDO::FETCH_ASSOC);

        $link = BASE_URL.'student-message-detail.php?course_id='.$_REQUEST['course_id'].'&student_id='.$_REQUEST['student_id'].'&instructor_id='.$_REQUEST['instructor_id'];
        ...
        header('location: '.BASE_URL.'instructor-message-detail.php?course_id='.$_REQUEST['course_id'].'&student_id='.$_REQUEST['student_id'].'&instructor_id='.$_REQUEST['instructor_id']);
        exit;
